...
func TestTemplateType_String(t *testing.T) {
	tests := []struct {
		...
		{"Senha Alterada Sucesso", TemplateTypeSenhaAlteradaSucesso, "SENHA_ALTERADA_SUCESSO"},
		{"Novo Dispositivo Autenticado", TemplateTypeNovoDispositivoAutenticado, "NOVO_DISPOSITIVO_AUTENTICADO"},
...
func TestTemplateType_IsValid(t *testing.T) {
	tests := []struct {
		...
		{"Senha Alterada Sucesso válido", TemplateTypeSenhaAlteradaSucesso, true},
		{"Novo Dispositivo Autenticado válido", TemplateTypeNovoDispositivoAutenticado, true},
