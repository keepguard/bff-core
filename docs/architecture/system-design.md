...
- **Endpoints Expostos Principais:** (prefixo `/api/v1`; health `/health`; métricas Prometheus em porta dedicada, ex. `9092/metrics`)
  ...
  - **Billing — troca de plano:** `GET /core/billing/subscriptions/:id/change-preview`, `POST .../change-plan`, `DELETE .../scheduled-change` (`billing:read`). O `change-plan` repete a regra `BILLING_PAYER_OPS` do subscribe: MANAGER sem ADMIN/SYSTEM recebe 403, porque quem opera a organização não troca plano de ninguém. `POST .../cancel` aceita `?immediate=true`, mas o BFF só repassa o flag quando o caller é admin — para o titular o cancelamento é sempre agendado.
